...
func debugf(format string, args ...any) {
	...
	log.Printf("[xconnect-mcp-debug] "+format, args...)
}
...
func main() {
	rootDir := os.Getenv("XCONNECT_ROOT")
	if rootDir == "" {
		wd, err := os.Getwd()
		if err != nil {
			panic(err)
	...
	debugMode = strings.EqualFold(strings.TrimSpace(os.Getenv("XCONNECT_MCP_DEBUG")), "true") ||
		strings.TrimSpace(os.Getenv("XCONNECT_MCP_DEBUG")) == "1"
	debugf("server start root=%s", absRoot)

	s := server.NewMCPServer("xconnect-local-mcp", "0.3.0")

	s.AddTool(
		mcp.NewTool("workspace_info", mcp.WithDescription("Return XConnect workspace metadata for MCP diagnostics.")),
	...
	s.AddTool(
		mcp.NewTool("macos_app_paths", mcp.WithDescription("Discover local macOS app support/config/log paths used by XConnect.")),
		func(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	s.AddTool(
		mcp.NewTool("macos_tail_logs",
			mcp.WithDescription("Tail log files under XConnect macOS logs directory."),
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if username == "" {
				username = strings.TrimSpace(os.Getenv("XCONNECT_ACCOUNTS_USERNAME"))
			}
			if password == "" {
				password = strings.TrimSpace(os.Getenv("XCONNECT_ACCOUNTS_PASSWORD"))
			}
			if username == "" || password == "" {
				return mcp.NewToolResultError("missing username/password; pass arguments or set XCONNECT_ACCOUNTS_USERNAME/XCONNECT_ACCOUNTS_PASSWORD"), nil
			}
			...
			if baseURL == "" {
				baseURL = strings.TrimSpace(os.Getenv("XCONNECT_ACCOUNTS_BASE_URL"))
			}
			if baseURL == "" {
	...
	// Build/debug helpers
	s.AddTool(mcp.NewTool("flutter_pub_get", mcp.WithDescription("Run flutter pub get in XConnect workspace.")),
		func(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			res := runCommand(ctx, absRoot, "flutter", "pub", "get")
			return jsonResult(res, !res.OK)
		},
	)
	s.AddTool(mcp.NewTool("flutter_analyze", mcp.WithDescription("Run flutter analyze in XConnect workspace.")),
		func(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func buildAppSupportCandidates(bundleID string) []string {
	...
	ids := []string{bundleID, "plus.svc.xconnect", "xconnect.svc.plus", "com.xconnect"}
	for _, id := range ids {
