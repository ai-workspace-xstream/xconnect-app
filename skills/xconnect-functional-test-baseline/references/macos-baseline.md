...
- `Disconnected` with `last stop reason Configuration was superceded by another configuration`:
  another System VPN replaced XConnect.
...
- `XConnect` is not required to be connected.
- Local listeners exist on `127.0.0.1:1080` or `127.0.0.1:1081`.
- System proxy output matches the active proxy configuration when XConnect manages system proxies.
