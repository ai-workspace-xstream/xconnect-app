...
This overview shows the current control path used by XConnect on each platform, with the Apple control plane called out explicitly because Packet Tunnel startup failures can happen before the provider or Go core is reached.
...
- For the full macOS implementation record, including process-level runtime shape, build artifacts, and file/function ownership, see [docs/macos-packet-tunnel-implementation.md](/Users/shenlan/workspaces/cloud-neutral-toolkit/xconnect.svc.plus/docs/macos-packet-tunnel-implementation.md).
...
- Apple Packet Tunnel startup and authorization still rely mainly on manual validation. Use [docs/macos-menubar-regression-checklist.md](/Users/shenlan/workspaces/cloud-neutral-toolkit/xconnect.svc.plus/docs/macos-menubar-regression-checklist.md) for current checks.
