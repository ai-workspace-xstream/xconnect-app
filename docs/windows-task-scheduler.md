...
1. 确保 `xray.exe` 已放在 `C:\Program Files\XConnect` 目录中。
2. 准备好相应配置文件，如 `C:\Program Files\XConnect\xray-vpn-node-jp.json`。
...
2. 触发器选择 **当计算机启动时**。
3. 操作选择 **启动程序**，并填写：
   - **程序或脚本**：`C:\Program Files\XConnect\xray.exe`
   - **添加参数**：`run -c "C:\Program Files\XConnect\xray-vpn-node-jp.json"`
