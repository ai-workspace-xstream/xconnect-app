...
- **🔴 S1 和 S2 必须绑定物理网卡的源地址**：调用时传入 `sourceAddress`，取主物理接口（例如 en0）的地址。R0 实测结论（`docs/report/macos-sandbox-spike-2026-09-19.md` §3.1）：
  - 只要有 TUN 在运行（别家的，或者**我们自己的**隧道），它会在本地直接完成所有 TCP 握手；
  - 连一个根本不存在的地址也会「0ms 连接成功」，测出来的数据全是假的；
  - 绑定到 en0 之后，`192.0.2.1` 正确超时，节点 RTT 为真实值 86ms。
  - 其他平台需要验证同样的做法：Android 用 `Network.bindSocket`（要走平台通道），Windows / Linux 绑定源地址。
- **哨兵探测**：
  - 每个诊断会话先对 `192.0.2.1:443`（RFC 5737 保留地址）发一次**不绑定源地址**的连接。
  - 如果能连上，就说明本机有 TUN 在本地应答。这时标记冲突 C1（`network-self-repair.md` §1.1），并且只采信绑定了物理网卡的样本。
  - 如果哨兵探测在**绑定物理网卡**的情况下也能连上，说明物理路径本身被拦截（例如透明代理），诊断结论要注明这一点。
- S3 仍然有意经过隧道，但**要校验应答内容**（HTTP 状态码或 204），不能只看连接是否建立。
- 结果分类：
